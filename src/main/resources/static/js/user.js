...
window.cadastrarUsuario = async function() {
    ...
    // Payload formatado para o novo backend em Inglês (User.java / RegisterDTO.java)
    const payload = {
        registration: registrationInput,
        name: nameInput,
        email: emailInput,
        password: passwordInput,
        permission: "technician" // Enum no backend
    };
    ...
    try {
        ...
        if (response.ok) {
            if (typeof abrirModalConfirmacao === "function") abrirModalConfirmacao();
